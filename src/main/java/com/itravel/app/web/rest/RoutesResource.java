...
import com.itravel.app.repository.RoutesRepository;
import com.itravel.app.repository.search.RoutesSearchRepository;
...
public class RoutesResource {
    ...
    @GetMapping("/routes")
    @Timed
    public List<Routes> getAllRoutes(@RequestParam(required = false) String filter) {
        if ("itinerary-is-null".equals(filter)) {
            log.debug("REST request to get all Routess where itinerary is null");
            return StreamSupport
                .stream(routesRepository.findAll().spliterator(), false)
                .filter(routes -> routes.getItinerary() == null)
